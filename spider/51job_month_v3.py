...
class SpiderDomainInfo(object):
    ...
    def start_spider(self):
        try:
            ...
            res = self.session.get(
                'https://search.51job.com/list/040000,000000,0000,00,3,99,%25E5%25A4%2596%25E8%25B4%25B8,2,1.html',
            ...
            if 'latin-1' not in str(start_spider_e):
                ...
                if 'gbk' not in str(start_spider_e):
                    ...
                    if '远程主机强迫关闭' not in str(start_spider_e):
                        time.sleep(1)
                        with open("log/51job_error_aborted.txt", 'a', encoding='utf-8') as f_err:
                            f_err.write(date_time_str + ":load_history_data 发生异常：" + str(start_spider_e) + '\n')
                    else:
                        pass
    ...
    def next_page(self, page):
        try:
            ...
            res = self.session.get(
                'https://search.51job.com/list/040000,000000,0000,00,3,99,%25E5%25A4%2596%25E8%25B4%25B8,2,' + str(
                    page) + '.html',
                data=boss_params_search,
                headers=search_header,
                proxies=None,
            ...
            if 'latin-1' not in str(next_page_e):
                ...
                if 'gbk' not in str(next_page_e):
                    ...
                    if '远程主机强迫关闭' not in str(next_page_e):
                        time.sleep(1)
                        with open("log/51job_error_aborted.txt", 'a', encoding='utf-8') as f_err:
                            f_err.write(date_time_str + ":load_history_data 发生异常：" + str(next_page_e) + '\n')
                    else:
                        pass
        # print("res.cookies:::" + str(res.cookies))
        # print("res.cookies:::" + str(res.headers))
        ...
        # print(detail_link_origin + ",完成")
            ...
            if 'latin-1' not in str(get_addr_detail_e):
                ...
                if 'gbk' not in str(get_addr_detail_e):
                    ...
                    if '远程主机强迫关闭' not in str(get_addr_detail_e):
                        time.sleep(1)
                        with open("log/51job_error_aborted.txt", 'a', encoding='utf-8') as f_err:
                            f_err.write(date_time_str + ":load_history_data 发生异常：" + str(get_addr_detail_e) + '\n')
                    else:
                        pass
        ...
        try:
            ...
            if 'latin-1' not in str(load_history_data_e):
                ...
                if 'gbk' not in str(load_history_data_e):
                    ...
                    if '远程主机强迫关闭' not in str(load_history_data_e):
                        time.sleep(1)
                        with open("log/51job_error_aborted.txt", 'a', encoding='utf-8') as f_err:
                            f_err.write(date_time_str + ":load_history_data 发生异常：" + str(load_history_data_e) + '\n')
                    else:
        ...
        try:
            ...
            if os.path.exists(root_file):
                ...
                if len(root_dirs) > 0:
                    ...
                    with open('files/51job_detail_uniq_company_' + date_time_str + '.txt', 'a',
    ...
    def entrance(self):
        try:
            ...
            print('加载到历史数据：%s:条', (self.uniq_company.__len__()))
            self.detail_to_one()
